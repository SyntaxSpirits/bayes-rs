...
fn main() -> Result<()> {
    ...
    for (i, group) in data.iter().enumerate() {
        println!("Group {}: {} observations, mean = {:.2}", 
                 i, group.len(), group.iter().sum::<f64>() / group.len() as f64);
    }
...
fn generate_hierarchical_data() -> Vec<Vec<f64>> {
    ...
    let group_means: Vec<f64> = (0..3)
        .map(|_| population_dist.sample(&mut rng))
        .collect();
    
    println!("True population mean: {:.2}, std: {:.2}", true_population_mean, true_population_std);
    println!("True group means: {:?}", group_means.iter().map(|x| format!("{:.2}", x)).collect::<Vec<_>>());
...
fn hierarchical_inference(data: &[Vec<f64>]) -> Result<Vec<DVector<f64>>> {
    ...
    println!("Warm-up completed. Acceptance rate: {:.3}", 
             sampler.acceptance_rate().unwrap_or(0.0));
    
    // Main sampling
    let samples = sampler.sample(8000);
    println!("Sampling completed. Final acceptance rate: {:.3}", 
             sampler.acceptance_rate().unwrap_or(0.0));
...
fn analyze_results(samples: &[DVector<f64>], data: &[Vec<f64>]) {
    ...
    for j in 0..n_groups {
        let empirical_mean = data[j].iter().sum::<f64>() / data[j].len() as f64;
        println!("  Group {} (θ_{}): {:.3} ± {:.3} [empirical: {:.3}]", 
                 j, j, 
    ...
    for j in 0..n_groups {
        ...
        println!("  Group {}: {:.1}% shrinkage toward population mean", j, shrinkage * 100.0);
    }
    ...
    for (i, name) in param_names.iter().enumerate() {
        println!("  {}: [{:.3}, {:.3}]", 
                 ...
                 diagnostics.quantiles[i][4]); // 97.5%
    }
}
...
mod tests {
    ...
    fn test_hierarchical_data_generation() {
        ...
        assert!(data.iter().all(|group| group.iter().all(|&x| x.is_finite())));
    }
